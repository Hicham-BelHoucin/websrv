...
/*   Updated: 2022/11/21 15:10:35 by obeaj            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
String response::MethodGet(LocationMap location, String path, String body)
{
    ...
    else if (mode & ResponseIUtils::ISFILE)
    {
        ...
        if (checkExtension(*it) != "php" || checkExtension(*it) != "py" || checkExtension(*it) != ".cgi")
        {
...
void response::setHeaders(request req)
{
    ...
    headers.insert(std::make_pair("Content-Type", getContentType(_body, _status_code)));
...
void response::ResponseBuilder()
{
    ...
    // if (!isCgiBody)
        _response += "\r\n";
    _response.append(stringtrim(_body));
    _response += "\r\n";
}
